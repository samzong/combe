...
$(GHOSTTY):
	...
	if [ -z "$$want" ]; then \
		echo 'error: minimum_zig_version missing from vendor/ghostty/build.zig.zon'; \
		exit 1; \
	fi; \
	if ! command -v zig >/dev/null 2>&1; then \
		if command -v brew >/dev/null 2>&1; then \
			printf 'zig %s not found; brew install zig\n' "$$want"; \
			brew install zig; \
		else \
			printf 'error: zig %s is required\n' "$$want"; \
			exit 1; \
		fi; \
	fi; \
	have=$$(zig version); \
	if [ "$$have" != "$$want" ]; then \
		printf 'error: vendor/ghostty needs zig %s, found %s\n' "$$want" "$$have"; \
		exit 1; \
	fi

build: ensure ## Debug build
	$(CARGO) build -p combe
...
check: ensure audit ## Full quality gate — dependency audit, format, lint, test
...
test: ensure ## Run workspace tests
	$(CARGO) test --workspace

lint: ensure ## Run clippy
	$(CARGO) clippy --workspace --all-targets -- -D warnings
...
app: ensure ## Build build/Combe.app
