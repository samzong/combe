...
use crate::glass::color;
use crate::habits;
use objc2::rc::Retained;
use objc2::{AnyThread, DefinedClass, MainThreadOnly, Message, define_class, msg_send};
use objc2_app_kit::{
    NSAccessibility, NSApplication, NSAutoresizingMaskOptions, NSBezierPath, NSButton, NSColor,
    NSEvent, NSEventType, NSFont, NSImage, NSImageView, NSLineBreakMode, NSTextField,
    NSTrackingArea, NSTrackingAreaOptions, NSView,
};
use objc2_foundation::{
    MainThreadMarker, NSMutableAttributedString, NSPoint, NSRange, NSRect, NSString,
};
    ...
    impl ClickView {
        ...
        fn draw_rect(&self, _dirty: NSRect) {
            ...
            if self.ivars().focus_visible.get() {
                ...
                let frame = rect(2.0, 2.0, (bounds.size.width - 4.0).max(0.0), (bounds.size.height - 4.0).max(0.0));
            ...
            if self.ivars().attention.get() && self.ivars().opened.get().is_none() {
                ...
                NSBezierPath::bezierPathWithOvalInRect(rect(self.bounds().size.width - 40.0, (self.bounds().size.height - SESSION_DOT) / 2.0, SESSION_DOT, SESSION_DOT)).fill();
            }
            ...
            NSBezierPath::bezierPathWithOvalInRect(rect(32.0, y, SESSION_DOT, SESSION_DOT))
            .fill();
...
impl ClickView {
    ...
    fn draw_marker(&self, marker: &str, x: f64, background: bool) {
        ...
        let frame = rect(x, (self.bounds().size.height - 20.0) / 2.0, 20.0, 20.0);
        if background {
    ...
    impl ActionButton {
        ...
        fn draw_rect(&self, dirty: NSRect) {
            if self.isHighlighted() || self.ivars().hovered.get() || self.state() != 0 {
                ...
                NSBezierPath::bezierPathWithOvalInRect(rect((bounds.size.width - diameter) / 2.0, (bounds.size.height - diameter) / 2.0, diameter, diameter)).fill();
            }
