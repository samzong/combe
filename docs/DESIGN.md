...
The tab bar's right-hand grid button, Cmd-Shift-Backslash, and View menu toggle the current workspace's overview. Cards show titles and every pane of each tab in its split layout, including panes hidden behind a zoom. Images are static, may lag behind background output, stay in memory, and are released when the overview closes. A surface without a frame shows its terminal background.

Opening focuses the active tab's card and scrolls it into view. Click enters a card's tab. Return, keypad Enter, and Space enter the focused card, or the tab that was active when the overview opened if focus has left the cards. Arrow keys move card focus only and never switch tabs: Left and Right stay inside the current row and stop at its ends; Up and Down move one row at the same column, and Down onto a shorter last row lands on that row's last card. Focus stops at the grid's outer edges and never wraps. Every move scrolls the focused card fully into view. Tab and Shift-Tab follow the window's key view loop. Escape and the toggle shortcut return without switching tabs and restore the prior responder if available. No reordering, closing, or cross-workspace aggregation. Other unmodified keys never reach terminals; Command shortcuts dismiss the overview before normal application routing. The prototype's grid button and Cmd-Shift-Backslash demonstrate these states.
